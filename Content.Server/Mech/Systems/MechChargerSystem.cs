...
public sealed class MechChargerSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        {
            if (!_powerCell.TryGetBatteryFromSlot(mechUid, out var mechBatteryUid, out var mechBattery))
                continue;

            var mechCharge = mechBattery.CurrentCharge;
            if (mechCharge <= 0)
                continue;
            ...
            foreach (var equipment in container.ContainedEntities)
            {
                ...
                var equipmentCharge = equipmentBattery.CurrentCharge;
                ...
                if (additionalNeeded > 0f)
                {
                    ...
                    if (toReserve > 0f && _battery.TryUseCharge(mechBatteryUid.Value, toReserve, mechBattery))
                    {
                        buffered += toReserve;
                ...
                if (transfer > 0f)
                {
                    ...
                    _battery.SetCharge(equipment, newCharge, equipmentBattery);
                    buffered -= transfer;
