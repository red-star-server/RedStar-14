...
public abstract partial class SharedMechSystem
{
    ...
    [PublicAPI]
    public virtual bool TryChangeEnergy(Entity<MechComponent?> ent, FixedPoint2 delta)
    {
        if (!Resolve(ent, ref ent.Comp, false))
            return false;
    ...
    /// </summary>
    [PublicAPI]
    public virtual void UpdateBatteryAlert(Entity<MechComponent?> ent)
    {
        if (!Resolve(ent, ref ent.Comp, false))
            return;

        _alerts.ClearAlert(ent.Owner, ent.Comp.NoBatteryAlert);
        _alerts.ShowAlert(ent.Owner, ent.Comp.BatteryAlert);
    }

    protected virtual bool HasEnergy(Entity<MechComponent?> ent)
    {
        return true;
    }
